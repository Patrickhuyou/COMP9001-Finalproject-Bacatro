...
def score_key(entry: Dict) -> tuple:
    # Higher level matters first, then remaining chips breaks ties.
...
class GameStorage:
    ...
    def read_save_slots(self) -> List[Optional[Dict]]:
        # Missing or broken save files are treated as empty slots.
        try:
    ...
    def save_game(self, data: Dict, slot: int = 0) -> None:
        # Save slots are stored together in one JSON list.
    ...
    def load_game(self, slot: int = 0) -> Dict:
        # Loading returns raw data; GameState.from_dict rebuilds the live object.
        if not 0 <= slot < SAVE_SLOT_COUNT:
            raise ValueError("Invalid save slot.")
    ...
    def read_leaderboard(self) -> List[Dict]:
        # Leaderboard is optional output, so failures simply return an empty list.
        try:
    ...
    def add_leaderboard_score(self, chips: int, level: int, rounds: int) -> None:
        # Keep only the best ten runs after inserting the new score.
